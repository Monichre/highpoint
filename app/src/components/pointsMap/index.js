...
lunar.addClass = function(elem, name) {
  !lunar.hasClass(elem, name) &&
    elem.setAttribute('class', (!!elem.getAttribute('class') ? elem.getAttribute('class') + ' ' : '') + name)
}
...
Point.prototype._updatePosition = function() {
  ...
  // Also update origins..
  this.el.style.transformOrigin = this.el.style.WebkitTransformOrigin =
    bbox.x + rect.width / 2 + 'px ' + (bbox.y + rect.height) + 'px'
...
Point.prototype._distanceToOpacity = function(d) {
  return Math.min(
    Math.max(lineEq(this.options.maxOpacity, 0, this.options.activeOn, this.dmax, d), 0),
...
function PointsMap(el, options) {
  ...
  if (this.bgElemsTotal <= 1) {
    return
  }

  // Points container.
  this.pointsWrapper = this.el.querySelector('.points')
  if (!this.pointsWrapper || getComputedStyle(this.pointsWrapper, null).display === 'none') {
    return
  }
  ...
  ;[].slice.call(this.pointsWrapper.querySelectorAll('.point')).forEach(function(point, pos) {
    ...
    var p = new Point(point, self.bgElems[pos], self.el, {
      ...
      onActive: function() {
        ...
        console.log('All the tooltips', self.tooltips)
        if (tooltip) {
          console.log(tooltip)
      ...
      onClick: function() {
        ...
        // Show Point´s bgEl.
        if(self.points[pos] && self.points[pos].bgEl) {
