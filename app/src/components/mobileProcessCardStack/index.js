...
import MobileProcessCardPreview from "../mobileProcessCardPreview";
import classie from "classie";
import "./_index.scss";

class MobileProcessCardStack extends Component {
  componentDidMount() {
    const dynamicScript = document.createElement("script"),
      mainScript = document.createElement("script"),
      helperScript = document.createElement("script"),
      modernizr = document.createElement("script");

    dynamicScript.src = "./dynamics.min.js";
    mainScript.src = "./stack.js";
    helperScript.src = "./helper.js";
    modernizr.src = "./modernizr-custom.js";

    const scripts = [modernizr, dynamicScript, mainScript, helperScript];
    ...
    const wira = new window.Stack(document.getElementById("stack_wira"));

    const buttonClickCallback = function(bttn) {
      var bttn = bttn || this;
      bttn.setAttribute("data-state", "unlocked");
    };
    document
      .querySelector(".button--accept[data-stack = stack_wira]")
      .addEventListener(clickeventtype, function() {
        wira.accept(buttonClickCallback.bind(this));
      });
    document
      .querySelector(".button--reject[data-stack = stack_wira]")
      .addEventListener(clickeventtype, function() {
  ...
  render() {
    return (
      <div className="card__stack">
        <ul id="stack_wira" className="stack stack--wira">
          {this.props.cards.map(card => (
            <MobileProcessCardPreview process={card} />
          ))}
        </ul>
        <div className="controls">
          <button
            className="button button--sonar button--reject"
            data-stack="stack_wira"
          >
            <i className="fa fa-times" />
            <span className="text-hidden">Reject</span>
          </button>
          <button
            className="button button--sonar button--accept"
            data-stack="stack_wira"
          >
...
export default MobileProcessCardStack;
